...
The benchmark is for a **separately generated and frozen held-out evaluation** of the selected semantic-verification model and final hybrid ClinDrift system. Unless a genuinely independent dataset creator/adjudicator is involved, the study will not describe the benchmark itself as independent. It is not a training set and it is not a clinical-validation dataset.
...
3. Keep the primary master's held-out benchmark synthetic. Openly licensed or authorised real-world material, if ever used later, must be treated as a separate external robustness dataset rather than silently mixed into the primary benchmark.
...
- **gold label**: contradiction, entailment, or neutral;\n- **scenario family**: the clinical/error scenario family used for leakage control;\n- **template family**: the wording/template lineage used to prevent trivial development-to-test reuse.
...
## Oracle-pair and evidence-pairing views

Two linked evaluation views are retained:

1. **Oracle-pair NLI benchmark** — the model receives the intended premise/hypothesis pair. This isolates inference quality.
2. **Evidence-pairing benchmark** — the automatic retriever must locate the relevant source/transformed evidence before NLI. This measures pairing recall@k separately from inference.

A system-level result should not attribute evidence-retrieval failures to NLI without reporting the pairing result.
...
3. confirm class counts, category counts, scenario-family counts and template-family counts;
